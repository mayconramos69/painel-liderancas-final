...
def lideranca():
    ...
    if int(user["pode_trabalho"] or 0) == 1:
...
def novo_trabalho():
    ...
    if titulo_info["valido"] is not True:
        ...
        flash("Cadastro não realizado: o número do título de eleitor é inválido.")
        return redirect(url_for("lideranca"))
...
def foto_trabalho(item_id):
    ...
    if session.get("perfil") == "admin":
        ...
        cur.execute("SELECT pode_trabalho FROM usuarios WHERE id=%s", (session.get("user_id"),))
        permissao = cur.fetchone()
        if not permissao or int(permissao["pode_trabalho"] or 0) != 1:
            cur.close(); conn.close()
            abort(403)
...
def lideranca_editar_trabalho(item_id):
    ...
    if not permissao or int(permissao["pode_trabalho"] or 0) != 1:
        cur.close(); conn.close()
        flash("Você não tem acesso ao Cadastro 02.")
        return redirect(url_for("lideranca"))

...
def admin():
    ...
    cur.execute("SELECT COUNT(*) AS c FROM trabalho WHERE titulo_valido IS DISTINCT FROM 1")
    ...
    if titulo_status == "validos":
        ...
        sql_titulos += " WHERE t.titulo_valido IS DISTINCT FROM 1"
...
def _converter_trabalho_para_espontaneo(item_id, lideranca_id=None):
    conn = db(); cur = conn.cursor()
    if lideranca_id is None:
        cur.execute("SELECT * FROM trabalho WHERE id=%s", (item_id,))
    else:
        cur.execute("SELECT * FROM trabalho WHERE id=%s AND lideranca_id=%s AND edit_liberado=1", (item_id, lideranca_id))
    item = cur.fetchone()
    if not item:
        cur.close(); conn.close()
        return False
    ...
    cur.execute("""
        INSERT INTO espontaneos
        (lideranca_id, nome_completo, municipio, telefone, endereco_completo,
         nome_normalizado, telefone_normalizado, created_at, edit_liberado)
        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,0)
    """, (item["lideranca_id"], item["nome"], item["municipio"], item["telefone"], item["endereco"],
          normalizar(item["nome"]), normalizar(item["telefone"]), datetime.now().isoformat()))
    cur.execute("DELETE FROM trabalho WHERE id=%s", (item_id,))
    conn.commit(); cur.close(); conn.close()
    return True


@app.route("/admin/trabalho_para_espontaneo/<int:item_id>", methods=["POST"])
@login_required
@admin_required
def admin_trabalho_para_espontaneo(item_id):
    if _converter_trabalho_para_espontaneo(item_id):
        flash("Cadastro movido de Trabalho para Espontâneo.")
    else:
        flash("Cadastro não encontrado.")
    return redirect(url_for("admin", aba="espontaneos"))


@app.route("/lideranca/trabalho_para_espontaneo/<int:item_id>", methods=["POST"])
@login_required
@lideranca_required
def lideranca_trabalho_para_espontaneo(item_id):
    if _converter_trabalho_para_espontaneo(item_id, session["user_id"]):
        flash("Cadastro movido para Espontâneo. A edição foi encerrada.")
    else:
        flash("Você não tem autorização para alterar este cadastro.")
    return redirect(url_for("lideranca"))

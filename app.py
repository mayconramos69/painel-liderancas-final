...
def analisar_titulo_eleitor(valor):
    ...
    soma1 = sum(numeros[i] * (i + 2) for i in range(8))
    resto1 = soma1 % 11
    dv1 = 0 if resto1 in (0, 1) else 11 - resto1

    soma2 = numeros[8] * 7 + numeros[9] * 8 + dv1 * 9
    resto2 = soma2 % 11
...
def novo_trabalho():
    ...
    cur.execute("""
        ...
        (lideranca_id, nome, municipio, colegio, endereco, telefone, zona, secao, numero_titulo,
         nome_normalizado, telefone_normalizado, titulo_normalizado, titulo_valido, titulo_uf, tipo_trabalho, votos_previstos, created_at)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
    """, (session["user_id"], nome, municipio, colegio, endereco, telefone, zona, secao, titulo, nn, tn, titn,
          None if titulo_info["valido"] is None else int(titulo_info["valido"]), titulo_info["uf"], tipo_trabalho, votos_previstos, datetime.now().isoformat()))
...
def editar_trabalho(item_id):
    ...
    if request.method == "POST":
        ...
        """, (nome, municipio, colegio, endereco, telefone, zona, secao, titulo, normalizar(nome), normalizar(telefone), normalizar(titulo),
              None if titulo_info["valido"] is None else int(titulo_info["valido"]), titulo_info["uf"], tipo_trabalho, votos_previstos, item_id))

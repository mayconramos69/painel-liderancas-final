...
def analisar_titulo_eleitor(valor):
    ...
    soma1 = sum(numeros[i] * pesos1[i] for i in range(8))
    resto1 = soma1 % 11
    dv1 = 0 if resto1 in (0, 1) else 11 - resto1

    soma2 = numeros[8] * 4 + numeros[9] * 3 + dv1 * 2
    resto2 = soma2 % 11
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS trabalho (
        ...
        titulo_uf TEXT,
        tipo_trabalho TEXT NOT NULL DEFAULT 'Trabalho de BU',
    ...
    cur.execute("ALTER TABLE trabalho ADD COLUMN IF NOT EXISTS tipo_trabalho TEXT NOT NULL DEFAULT 'Trabalho de BU'")
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS eleitoral_municipios (
        id SERIAL PRIMARY KEY,
        municipio TEXT UNIQUE NOT NULL,
        votos_2018 INTEGER NOT NULL DEFAULT 0,
        votos_2022 INTEGER NOT NULL DEFAULT 0,
        meta_2026 INTEGER NOT NULL DEFAULT 0,
        observacao TEXT,
...
def novo_trabalho():
    ...
    tipo_trabalho = request.form.get("tipo_trabalho", "Trabalho de BU").strip()
    if tipo_trabalho not in ("Líder de rua", "Trabalho de BU"):
        tipo_trabalho = "Trabalho de BU"
    try:
...
def admin():
    ...
    cur.execute("SELECT COUNT(*) AS c FROM trabalho WHERE tipo_trabalho='Trabalho de BU' OR tipo_trabalho IS NULL OR tipo_trabalho=''")
    ...
    cur.execute("""
        SELECT COALESCE(NULLIF(TRIM(u.municipio),''),'Não informado') AS municipio,
               COUNT(*) AS liderancas,
               COALESCE(SUM(u.votos_estimados),0) AS estimativa_liderancas,
               COALESCE((SELECT COUNT(*) FROM espontaneos e WHERE LOWER(TRIM(e.municipio))=LOWER(TRIM(COALESCE(NULLIF(TRIM(u.municipio),''),'Não informado')))),0) AS espontaneos,
               COALESCE((SELECT COUNT(*) FROM trabalho t WHERE LOWER(TRIM(t.municipio))=LOWER(TRIM(COALESCE(NULLIF(TRIM(u.municipio),''),'Não informado')))),0) AS trabalho
        FROM usuarios u WHERE u.perfil='lideranca'
        GROUP BY COALESCE(NULLIF(TRIM(u.municipio),''),'Não informado')
        ORDER BY estimativa_liderancas DESC, liderancas DESC
    """)
    cobertura_municipios = cur.fetchall()
    cur.execute("SELECT * FROM eleitoral_municipios ORDER BY municipio")
    historico_municipios = cur.fetchall()
    historico_por_nome = {normalizar(x['municipio']): x for x in historico_municipios}
    inteligencia = []
    nomes = {normalizar(x['municipio']): x['municipio'] for x in cobertura_municipios}
    nomes.update({normalizar(x['municipio']): x['municipio'] for x in historico_municipios})
    cobertura_por_nome = {normalizar(x['municipio']): x for x in cobertura_municipios}
    for chave, nome in sorted(nomes.items(), key=lambda kv: kv[1]):
        c = cobertura_por_nome.get(chave, {})
        h = historico_por_nome.get(chave, {})
        inteligencia.append({
            'municipio': nome,
            'liderancas': int(c.get('liderancas',0) or 0),
            'estimativa_liderancas': int(c.get('estimativa_liderancas',0) or 0),
            'espontaneos': int(c.get('espontaneos',0) or 0),
            'trabalho': int(c.get('trabalho',0) or 0),
            'votos_2018': int(h.get('votos_2018',0) or 0),
            'votos_2022': int(h.get('votos_2022',0) or 0),
            'meta_2026': int(h.get('meta_2026',0) or 0),
    ...
    return render_template(
        ...
        historico_municipios=historico_municipios
    )


@app.route("/admin/inteligencia/salvar", methods=["POST"])
@login_required
@admin_required
def salvar_inteligencia():
    municipio = request.form.get("municipio", "").strip()
    if not municipio:
        flash("Informe o município.")
        return redirect(url_for("admin") + "#inteligencia")
    def inteiro(nome):
        try: return max(0, int(request.form.get(nome, 0) or 0))
        except ValueError: return 0
    conn=db(); cur=conn.cursor()
    cur.execute("""INSERT INTO eleitoral_municipios (municipio,votos_2018,votos_2022,meta_2026,observacao,updated_at)
        VALUES (%s,%s,%s,%s,%s,%s)
        ON CONFLICT (municipio) DO UPDATE SET votos_2018=EXCLUDED.votos_2018,votos_2022=EXCLUDED.votos_2022,meta_2026=EXCLUDED.meta_2026,observacao=EXCLUDED.observacao,updated_at=EXCLUDED.updated_at""",
        (municipio,inteiro('votos_2018'),inteiro('votos_2022'),inteiro('meta_2026'),request.form.get('observacao','').strip(),datetime.now().isoformat()))
    conn.commit(); cur.close(); conn.close(); flash("Dados eleitorais atualizados.")
    return redirect(url_for("admin") + "#inteligencia")
...
def lideranca_editar_trabalho(item_id):
    ...
    if request.method == "POST":
        ...
        tipo_trabalho = request.form.get("tipo_trabalho", "Trabalho de BU").strip()
        if tipo_trabalho not in ("Líder de rua", "Trabalho de BU"): tipo_trabalho = "Trabalho de BU"
        try: votos_previstos = max(0, int(request.form.get("votos_previstos", "0") or 0))
        except ValueError: votos_previstos = 0
        titulo_info = analisar_titulo_eleitor(titulo)
        if titulo_info["valido"] is not True:
            cur.close(); conn.close(); flash("Alteração não salva: o título informado é inválido.")
        ...
        cur.execute("""UPDATE trabalho SET nome=%s, municipio=%s, colegio=%s, endereco=%s, telefone=%s,
                    ...
                    (nome, municipio, colegio, endereco, telefone, zona, secao, titulo, normalizar(nome),
                     normalizar(telefone), normalizar(titulo), int(titulo_info["valido"]), titulo_info["uf"],
...
def exportar_word(tipo):
    ...
    if tipo == "espontaneos":
        ...
        titulo, nome = "Contatos espontâneos", "contatos_espontaneos_editavel.docx"
    elif tipo == "trabalho":
        cur.execute("""SELECT u.nome AS lideranca, t.nome, t.municipio, t.colegio, t.telefone, t.zona, t.secao,
        ...
        titulo, nome = "Cadastros de trabalho", "cadastros_trabalho_editavel.docx"
...
def editar_trabalho(item_id):
    ...
    if request.method == "POST":
        ...
        if titulo_info["valido"] is not True:
            cur.close(); conn.close()
            flash("Alteração não salva: o número do título de eleitor é inválido.")
        ...
        cur.execute("""
            ...
            WHERE id=%s
        """, (nome, municipio, colegio, endereco, telefone, zona, secao, titulo, normalizar(nome), normalizar(telefone), normalizar(titulo),

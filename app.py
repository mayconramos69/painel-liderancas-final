...
def init_db():
    ...
    cur.execute("ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS valor_acordado NUMERIC(12,2) NOT NULL DEFAULT 0")
    cur.execute("ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS valor_pago NUMERIC(12,2) NOT NULL DEFAULT 0")
...
def admin():
    ...
    cur.execute("""
        SELECT COALESCE(SUM(valor_acordado),0) AS acordado,
               COALESCE(SUM(valor_pago),0) AS pago
        FROM usuarios WHERE perfil='lideranca'
    """)
    valores = cur.fetchone()
    total_acordado = valores["acordado"] or 0
    total_pago = valores["pago"] or 0
    total_pendente_valores = total_acordado - total_pago
    ...
    return render_template(
        ...
        total_pendente_valores=total_pendente_valores
    )
...
@app.route("/admin/valores/<int:user_id>", methods=["POST"])
@login_required
@admin_required
def atualizar_valores(user_id):
    valor_acordado = moeda_para_decimal(request.form.get("valor_acordado"))
    valor_pago = moeda_para_decimal(request.form.get("valor_pago"))
    conn = db()
    cur = conn.cursor()
    cur.execute("""
        UPDATE usuarios SET valor_acordado=%s, valor_pago=%s
        WHERE id=%s AND perfil='lideranca'
    """, (valor_acordado, valor_pago, user_id))
    conn.commit()
    cur.close()
    conn.close()
    flash("Valores da liderança atualizados.")
    return redirect(url_for("admin"))
...
def exportar_pdf(tipo):
    ...
    if tipo == "espontaneos":
        ...
        cur.execute("""SELECT u.nome AS lideranca, t.nome, t.municipio, t.colegio, t.telefone, t.zona, t.secao, t.numero_titulo
                       FROM trabalho t JOIN usuarios u ON u.id=t.lideranca_id ORDER BY t.id DESC""")
        rows=cur.fetchall()
        linhas=[[r["lideranca"],r["nome"],r["municipio"],r["colegio"],r["telefone"],r["zona"],r["secao"],r["numero_titulo"]] for r in rows]
        pdf=gerar_pdf("Contatos para trabalho", ["Liderança","Nome","Município","Colégio","Telefone","Zona","Seção","Título"], linhas, [88,110,78,105,82,46,46,84])
        nome="contatos_trabalho.pdf"
    elif tipo == "valores":
        cur.execute("""SELECT nome, municipio, valor_acordado, valor_pago, (valor_acordado-valor_pago) AS saldo
                       FROM usuarios WHERE perfil='lideranca' ORDER BY nome""")
        rows=cur.fetchall()
        linhas=[[r["nome"],r["municipio"],f'R$ {r["valor_acordado"]:,.2f}',f'R$ {r["valor_pago"]:,.2f}',f'R$ {r["saldo"]:,.2f}'] for r in rows]
        pdf=gerar_pdf("Valores das lideranças", ["Liderança","Município","Valor definido","Valor já pago","Saldo"], linhas, [180,130,110,110,110], orientacao=A4)
        nome="valores_liderancas.pdf"
    else:
